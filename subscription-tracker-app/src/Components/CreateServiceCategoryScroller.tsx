...
const CreateServiceCategoryScroller = ({
  ...
  return (
    <View style={styles.row}>
      <Text style={S.headingTwo}>Klass:</Text>
      <View style={[styles.row, { gap: 16, marginLeft: 24 }]}>
        <Pressable onPress={() => handleButtonClickLeft}>
          <ArrowLeft height={24} width={24} color={S.onBackgroundText.color} />
        </Pressable>
        <Text style={S.headingTwo} ref={ref}>
          {categories[placement]?.name}
        </Text>
        <Pressable onPress={() => handleButtonClickRight}>
          <ArrowRight height={24} width={24} color={S.onBackgroundText.color} />
        </Pressable>
      </View>
    </View>
  );
